...
echo -e "${YELLOW}************ Select SaoSao Version Apache Web Server************"
...
sudo echo '#SaoSao' > /var/www/SaoSaoport
...
sudo bash -c "$(curl -Lfo- https://raw.githubusercontent.com/hosseinabdinasab/Nethogs-Json-main/master/install.sh)"
sudo mysql -e "create database SaoSao_plus;" &
wait
...
then
 sudo mysql -e "USE SaoSao_plus; UPDATE admins SET username = '${adminusername}' where permission='admin';"
 sudo mysql -e "USE SaoSao_plus; UPDATE admins SET password = '${adminpassword}' where permission='admin';"
 sudo mysql -e "USE SaoSao_plus; UPDATE settings SET ssh_port = '${port}' where id='1';"
else
sudo mysql -e "USE SaoSao_plus; INSERT INTO admins (username, password, permission, credit, status) VALUES ('${adminusername}', '${adminpassword}', 'admin', '', 'active');"
sudo home_url=$protcohttp://${defdomain}:$sshttp
sudo mysql -e "USE SaoSao_plus; INSERT INTO settings (ssh_port, tls_port, t_token, t_id, language, multiuser, ststus_multiuser, home_url) VALUES ('${port}', '444', '', '', '', 'active', '', '${home_url}');"
fi
...
wait
sudo curl -o /root/SaoSao.sh https://raw.githubusercontent.com/hosseinabdinasab/SaoSao-Panel/master/cli.sh
sudo wget -4 -O /usr/local/bin/SaoSao https://raw.githubusercontent.com/hosseinabdinasab/SaoSao-Panel/master/cli.sh
sudo chmod +x /usr/local/bin/SaoSao 
