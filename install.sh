...
[ -f "$source_file" ] && cp "$source_file" "$destination_directory" && echo "index.php copied successfully."
  ...
  if dpkg-query -W -f='${Status}' mariadb-server 2>/dev/null | grep -q "install ok installed"; then
    adminuser=$(mysql -N -e "use SaoSao_plus; select username from admins where permission='admin';")
    adminpass=$(mysql -N -e "use SaoSao_plus; select username from admins where permission='admin';")
    ssh_tls_port=$(mysql -N -e "use SaoSao_plus; select tls_port from settings where id='1';")
  fi
  ...
  if test -f "/var/www/SaoSaoport"; then
    domainp=$(cat /var/www/SaoSaoport | grep "^DomainPanel")
    sslp=$(cat /var/www/SaoSaoport | grep "^SSLPanel")
    xpo=$(cat /var/www/SaoSaoport | grep "^SaoSaoport")
    xport=$(echo "$xpo" | sed "s/SaoSaoport //g")
...
wellcomeINSTALL() {
  echo -e "${YELLOW}************ Select SaoSao Version Nginx Web Server************"
echo -e "${GREEN}  1)SaoSao v1.0.0"
echo -ne "${GREEN}\nSelect Version : ${ENDCOLOR}" ;read n
  read n
  if [ "$n" != "" ]; then
    if [ "$n" == "1" ]; then
linkd=https://api.github.com/repos/hosseinabdinasab/SaoSao-Panel/releases/tags/v1-0-0
fi
...
EOF
    ...
    if test -f "/var/www/SaoSaoport"; then
      echo "File exists SaoSaoport"
    else
      touch /var/www/SaoSaoport
    fi
    link=$(sudo curl -Ls "$linkd" | grep '"browser_download_url":' | sed -E 's/.*"([^"]+)".*/\1/')
    sudo wget -O /var/www/html/update.zip $link
    sudo unzip -o /var/www/html/update.zip -d /var/www/html/ &
    wait
    sudo wget -4 -O /usr/local/bin/cronx https://raw.githubusercontent.com/hosseinabdinasab/SaoSao-Panel/master/cronx
    chmod +x /usr/local/bin/cronx
    sudo wget -4 -O /usr/local/bin/cronxfixed https://raw.githubusercontent.com/hosseinabdinasab/SaoSao-Panel/master/cronxfixed
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/hosseinabdinasab/SaoSao-Panel/master/ioncube.sh --ipv4)
    wait
    ...
    # Random port number generator to prevent SaoSao detection by potential attackers
    randomPort=""
    ...
    read udpport
    sudo bash -c "$(curl -Ls https://raw.githubusercontent.com/hosseinabdinasab/Nethogs-Json-main/master/install.sh --ipv4)"
...
EOF
    ...
    echo '#SaoSao' >/var/www/SaoSaoport
    sudo sed -i -e '$a\'$'\n''SaoSaoport '$serverPort /var/www/SaoSaoport
    wait
    ...
    sudo wget -q -O /usr/local/bin/wss https://raw.githubusercontent.com/hosseinabdinasab/SaoSao-Panel/master/wss
    sudo chmod +x /usr/local/bin/wss
    sudo wget -q -O /usr/local/bin/wssd https://raw.githubusercontent.com/hosseinabdinasab/SaoSao-Panel/master/wssd
    sudo chmod +x /usr/local/bin/wssd

    # Installing Service
    cat >/etc/systemd/system/wss.service <<END
[Unit]
Description=Python Proxy SaoSao
Documentation=https://t.me/saosao_ch
...
checkDATABASE() {
  mysql -e "create database SaoSao_plus;" &
  wait
  ...
  if [ -n "$adminuser" -a "$adminuser" != "NULL" ]; then
    mysql -e "USE SaoSao_plus; UPDATE admins SET username = '${adminusername}' where permission='admin';"
    mysql -e "USE SaoSao_plus; UPDATE admins SET password = '${adminpassword}' where permission='admin';"
    mysql -e "USE SaoSao_plus; UPDATE settings SET ssh_port = '${port}' where id='1';"
    ...
    mysql -e "USE SaoSao_plus; INSERT INTO admins (username, password, permission, credit, status) VALUES ('${adminusername}', '${adminpassword}', 'admin', '', 'active');"
    home_url=$protcohttp://${defdomain}:$sshttp
    mysql -e "USE SaoSao_plus; INSERT INTO settings (ssh_port, tls_port, t_token, t_id, language, multiuser, ststus_multiuser, home_url) VALUES ('${port}', '444', '', '', '', 'active', '', '${home_url}');"
  fi
}
moreCONFIG() {
...
ENDOFFILE
  ...
  sudo mkdir -p /SaoSao
  wait
  curl -sL -o /usr/local/bin/xp_user_limit https://raw.githubusercontent.com/hosseinabdinasab/SaoSao-Panel/master/xp_user_limit.sh
  ...
  wait
  curl -o /root/SaoSao.sh https://raw.githubusercontent.com/hosseinabdinasab/SaoSao-Panel/master/cli.sh
  sudo wget -4 -O /usr/local/bin/SaoSao https://raw.githubusercontent.com/hosseinabdinasab/SaoSao-Panel/master/cli.sh
  chmod +x /usr/local/bin/SaoSao
...
endINSTALL() {
  echo -e "************ SaoSao ************ \n"
  echo -e "SaoSao Link : $protcohttp://${defdomain}:$sshttp/login"
