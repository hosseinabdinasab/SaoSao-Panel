...
xpd=$(cat /var/www/SaoSaoport | grep "^DomainPanel")
...
if [ "$xpdomain" != "" ]; then
sed -i 's/$xpdomain/$domain/' /var/www/SaoSaoport
else
sudo sed -i -e '$a\'$'\n''DomainPanel '$domain /var/www/SaoSaoport
sudo sed -i -e '$a\'$'\n''SSLPanel True' /var/www/SaoSaoport
fi
wait
