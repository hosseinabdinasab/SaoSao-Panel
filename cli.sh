...
sshport=$(mysql -N -e "use SaoSao_plus; select ssh_port from settings where id='1';")
ssh_tls_port=$(mysql -N -e "use SaoSao_plus; select tls_port from settings where id='1';")
if [ -f "/var/www/SaoSaoport" ]; then
domain=$(cat /var/www/SaoSaoport | grep "^DomainPanel")
ssl=$(cat /var/www/SaoSaoport | grep "^SSLPanel")
panelport=$(cat /var/www/SaoSaoport | grep "^SaoSaoport")
panelport=$(echo "$panelport" | sed "s/SaoSaoport //g")
...
function show_menu() {
	...
	echo -e "${GREEN} .----------------. .----------------. .----------------. .----------------. .----------------. .----------------. "
	echo -e "${GREEN}| .--------------. | .--------------. | .--------------. | .--------------. | .--------------. | .--------------. |"
	echo -e "${GREEN}| |    _______   | | |      __      | | |     ____     | | |    _______   | | |      __      | | |     ____     | |"
	echo -e "${GREEN}| |   /  ___  |  | | |     /  \     | | |   .'    `.   | | |   /  ___  |  | | |     /  \     | | |   .'    `.   | |"
	echo -e "${GREEN}| |  |  (__ \_|  | | |    / /\ \    | | |  /  .--.  \  | | |  |  (__ \_|  | | |    / /\ \    | | |  /  .--.  \  | |"
	echo -e "${GREEN}| |   '.___`-.   | | |   / ____ \   | | |  | |    | |  | | |   '.___`-.   | | |   / ____ \   | | |  | |    | |  | |"
	echo -e "${GREEN}| |  |`\____) |  | | | _/ /    \ \_ | | |  \  `--'  /  | | |  |`\____) |  | | | _/ /    \ \_ | | |  \  `--'  /  | |"
	echo -e "${GREEN}| |  |_______.'  | | ||____|  |____|| | |   `.____.'   | | |  |_______.'  | | ||____|  |____|| | |   `.____.'   | |"
	echo -e "${GREEN}| |              | | |              | | |              | | |              | | |              | | |              | |"
	echo -e "${GREEN}| '--------------' | '--------------' | '--------------' | '--------------' | '--------------' | '--------------' |"
	echo -e "${GREEN} '----------------' '----------------' '----------------' '----------------' '----------------' '----------------' "
	echo -e "$BLUE┌────────────────────────────────────────────────────────────────•${NC}"
	...
	echo -e "$BLUE│$NC ${ORANGE}•${NC} SaoSao Link: \"http://$domain:$def_port/$def_cp/login\""
	...
	echo -e " ${BLUE}[4]${NC} ${ORANGE}•${NC} ${WHITE}Update SaoSao Nginx Web Server${NC}"
	echo -e " ${BLUE}[5]${NC} ${ORANGE}•${NC} ${WHITE}Remove SaoSao${NC}"
	echo -e " ${BLUE}[6]${NC} ${ORANGE}•${NC} ${WHITE}Remove All Admin SaoSao${NC}"
...
function select_option() {
    ...
    case $choice in
        1)
            ...
            mysql -e "USE SaoSao_plus; UPDATE admins SET username = '${username}' where id='1';"
            mysql -e "USE SaoSao_plus; UPDATE admins SET password = '${password}' where id='1';"
            ;;
        2)
...
connect = 0.0.0.0:$sshport
            ...
            mysql -e "USE SaoSao_plus; UPDATE settings SET tls_port = '${tlsport}' where id='1';"
            reboot
            ;;
        4)
            bash <(curl -Ls https://github.com/hosseinabdinasab/SaoSao-Panel/raw/master/install.sh --ipv4)
            ;;
        5)
        ...
        6)
       mysql -e "USE SaoSao_plus; TRUNCATE TABLE admins;"
       echo "Removed All Admin"
        ;;
        
        7)
        bash <(curl -Ls https://github.com/hosseinabdinasab/SaoSao-Panel/raw/master/block_iran.sh --ipv4)
        ;;
        8)
        sudo iptables -F
        ;;
        9)
        bash <(curl -Ls https://raw.githubusercontent.com/hosseinabdinasab/Dropbear-ssh/main/install.sh)
        ;;
        10)
        bash <(curl -Ls https://raw.githubusercontent.com/hosseinabdinasab/SaoSao-Panel/master/wp-install.sh --ipv4)
        ;;
        11)
        bash <(curl -Ls https://raw.githubusercontent.com/hosseinabdinasab/SaoSao-Panel/master/fix-call.sh --ipv4)
        ;; 
        12)
        bash <(curl -Ls https://raw.githubusercontent.com/hosseinabdinasab/SaoSao-Panel/master/OT/singbox.sh --ipv4)
        ;;
	13)
